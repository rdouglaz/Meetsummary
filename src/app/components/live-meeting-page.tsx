...
export function LiveMeetingPage({ onNavigate }: LiveMeetingPageProps) {
  ...
  const startMeeting = useCallback(async (source: LiveSettings['source'], liveSettings: LiveSettings) => {
    ...
    trackEvent('live_session_started', { source, meetingId: meeting.id, sessionId: session.id });
    setPhase('live');
  ...
  const endMeeting = async () => {
    ...
    if (sessionId && meetingId) {
      try {
        ...
        trackEvent('live_session_ended', { meetingId, durationSecs: Math.round(elapsed) });
        updateSuccess = true;
